...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        # image - иконка кнопки
        btn_open_dialog = tk.Button(toolbar, bg='blue', bd=0,
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'tel', 'email', 'income'),
        ...
        self.tree.heading("income", text='Зарплата')

        # упаковка
        self.tree.pack(side=tk.LEFT)

        scroll = tk.Scrollbar(self, command=self.tree.yview, bg='orange')
        scroll.pack(side=tk.LEFT, fill=tk.Y)
        self.tree.configure(yscrollcommand=scroll.set)

    # добавление данных
    def records(self, name, tel, email, income):
        self.db.insert_data(name, tel, email, income)
        self.view_records()

    # обновление (изменение) данных
    def update_record(self, name, tel, email, income):
        self.db.c.execute('''UPDATE db SET name=?, tel=?, email=?, income=? WHERE ID=?''',
                          (name, tel, email, income, self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        # подписи
        label_name = tk.Label(self, text='ФИО:', bg='black', fg='white', font='10')
        label_name.place(x=50, y=30)
        label_select = tk.Label(self, text='Телефон', bg='black', fg='white', font='10')
        label_select.place(x=50, y=60)
        label_sum = tk.Label(self, text='E-mail', bg='black', fg='white', font='10')
        label_sum.place(x=50, y=90)
        label_income = tk.Label(self, text='ЗП', bg='black', fg='white', font='10')
        label_income.place(x=50, y=120)
        ...
        self.entry_income = ttk.Entry(self)
        self.entry_income.place(x=200, y=120)
...
class Update(Child):
    ...
    def default_data(self):
        ...
        self.entry_income.insert(0, row[4])
...
class Search(tk.Toplevel):
    ...
    def init_search(self):
        ...
        label_search = tk.Label(self, text='Поиск', bg='black', fg='white', font='10')
...
class DB:
    def __init__(self):
        ...
        self.c.execute(
            '''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text, income text)''')
        # сохранение изменений БД
        self.conn.commit()

    # метод добавления в БД
    def insert_data(self, name, tel, email, income):
        self.c.execute('''INSERT INTO db (name, tel, email, income) VALUES (?, ?, ?, ?)''',
                       (name, tel, email, income))
        self.conn.commit()
